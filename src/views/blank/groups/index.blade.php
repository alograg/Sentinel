...
<table class="table table-striped table-hover">
	...
	@foreach ($groups as $group)
		<tr>
			...
			<td>
				<button onClick="location.href='{{ action('GroupController@edit', array($group->id)) }}'">Edit</button>
			 	<button class="action_confirm {{ ($group->id == 2) ? 'disabled' : '' }}" type="button" data-method="delete" href="{{ URL::to('groups') }}/{{ $group->id }}">Delete</button>
			 </td>
		</tr>	
	@endforeach
	</tbody>
